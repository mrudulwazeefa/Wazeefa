...
function setDropdownScrollLock(enable) {
    if (enable && !dropdownLockState.locked) {
        ...
        window.addEventListener("touchmove", preventDropdownScroll, { passive: false });
        window.addEventListener("keydown", preventDropdownScroll, { passive: false });
    }

    if (!enable && dropdownLockState.locked) {
        dropdownLockState.locked = false;
        window.removeEventListener("wheel", preventDropdownScroll, { passive: false });
        window.removeEventListener("touchmove", preventDropdownScroll, { passive: false });
        window.removeEventListener("keydown", preventDropdownScroll, { passive: false });
    ...
    headerDropdownGroups.forEach((group) => {
        ...
        const dropdownTimeline = gsap.timeline({ paused: true });
            ...
            if (dropdownContainer && dropdownLinks.length) {
                ...
                dropdownTimeline
                    .to(dropdownImage, { opacity: 1, y: 0, duration: 0.48, ease: "power2.out" }, 0)
                    .to(dropdownLinks, {
                        opacity: 1,
                        y: 0,
                        duration: 0.42,
                ...
                group.addEventListener("mouseleave", () => {
                    ...
                    megaMenuHoverTimeout = setTimeout(() => {
                        const navHovered = navDropdown?.matches(":hover");
                        const panelHovered = megaMenuPanel?.matches(":hover");
                        const groupHovered = Array.from(headerDropdownGroups).some((g) => g.matches(":hover"));
...
function initHomePage() {
    ...
    const aboutContentStates = [
        { heading: "We build sharp digital products with strategy and clean execution.", lines: ["We combine strategy, design, and engineering to create websites, apps, and digital solutions that deliver seamless experiences and measurable business results."], ctaText: "Discover more" },
        { heading: "Every interface is shaped to move naturally, tell a story, and support growth.", lines: ["We focus on performance, intuitive experiences, and purposeful design to create digital products that look refined, engage users, and deliver results."], ctaText: "Discover more" },
    ];
    ...
    function initMobileServiceSection() {
        ...
        const serviceData = [
            { title: "Website Development", p: "Building responsive, high-performance websites that strengthen your brand, engage visitors, and support business growth.", href: "web-development.php", media: '<video class="h-full w-full object-cover" autoplay muted loop playsinline><source src="assets/video/web.mp4" type="video/mp4"></video>' },
            { title: "Ecommerce Development", p: "Creating conversion-focused eCommerce platforms that deliver seamless shopping experiences and support long-term growth.", href: "ecommerce-website-development.php", media: '<img src="https://mir-s3-cdn-cf.behance.net/projects/max_808_webp/d754be230511570.Y3JvcCwxOTIxLDE1MDMsNCww.jpg" class="h-full w-full object-cover" />' },
            { title: "Mobile App Development", p: "Designing intuitive mobile applications that enhance user experiences, increase engagement, and drive business value.", media: '<video class="h-full w-full object-cover" autoplay muted loop playsinline><source src="assets/video/mobile.mp4" type="video/mp4"></video>' },
            { title: "CMS Development", p: "Driving visibility, engagement, and growth through SEO, social media, content marketing, and performance-driven digital strategies.", href: "cms-development.php", media: '<img src="https://mir-s3-cdn-cf.behance.net/projects/max_808_webp/f850ce239041909.Y3JvcCwxOTM5LDE1MTYsMjczLDI0NA.jpg" class="h-full w-full object-cover" />' },
            { title: "Software Development", p: "Building custom software solutions that streamline operations, improve efficiency, and scale with your business.", href: "software-development.php", media: '<img src="https://mir-s3-cdn-cf.behance.net/projects/max_808_webp/4aa307232271915.Y3JvcCwzMjMyLDI1MjgsOCww.jpg" class="h-full w-full object-cover" />' },
            { title: "Branding", p: "Developing strategic brand identities that help businesses stand out, build trust, and connect with the right audience.", href: "branding.php", media: '<img src="https://cdn.dribbble.com/userupload/16983506/file/original-16fe9f7d37205a29fa0a00cdc98e4fa0.png?resize=752x&vertical=center" class="h-full w-full object-cover" />' },
        ];
...
function initBrandingPage() {
    ...
    tl
      ...
      .to(shape1, { opacity: 0, rotation: transitionDegrees, duration: transitionTime, ease: "power3.inOut" }, "sync1")
      .to(shape2, { opacity: 1, rotation: 0, duration: transitionTime, ease: "power3.inOut" }, "sync1")
      .to({}, { duration: holdTime })
      .set(shape3, { rotation: -transitionDegrees }, "sync2")
      .to(shape2, { opacity: 0, rotation: transitionDegrees, duration: transitionTime, ease: "power3.inOut" }, "sync2")
      .to(shape3, { opacity: 1, rotation: 0, duration: transitionTime, ease: "power3.inOut" }, "sync2")
      .to({}, { duration: holdTime })
      .set(shape1, { rotation: -transitionDegrees }, "sync3")
      .to(shape3, { opacity: 0, rotation: transitionDegrees, duration: transitionTime, ease: "power3.inOut" }, "sync3")
      .to(shape1, { opacity: 1, rotation: 0, duration: transitionTime, ease: "power3.inOut" }, "sync3");
}
